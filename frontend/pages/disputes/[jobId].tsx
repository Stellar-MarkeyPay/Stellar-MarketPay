    ...
    >
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-amber-800 mt-0.5">
          {(ev.fileSize / 1024).toFixed(1)} KB · {shortenAddress(ev.uploaderAddress)} ·{" "}
...
function validateFile(file: File): string | null {
  if (!ALLOWED_TYPES.includes(file.type))
    return "Only images, PDFs, and plain text files are allowed.";
...
export default function DisputePage({ publicKey }: PageProps) {
  ...
  const myEvidentCount = (detail?.evidence ?? []).filter(
    (ev) => ev.uploaderAddress === publicKey
  ).length;
  ...
  const enqueueFiles = useCallback(
    (files: File[]) => {
      ...
      for (const file of files) {
        ...
        if (slotsLeft - toAdd.length <= 0) {
          setDropError("Maximum 10 files per party.");
          break;
        }
        toAdd.push({
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-amber-800">
              Job created · {new Date(job.created_at).toLocaleDateString()}
            </span>
          </div>
          <div className="flex items-center gap-3">
      ...
      {isParty && (
        <div className="card space-y-4 max-w-lg">
          ...
          <div
            ...
            className={clsx(
              ...
              isDragOver
                ...
                : "border-amber-900/40 hover:border-market-500/50",
          ...
          {pendingFiles.length > 0 && (
            <ul className="space-y-2" aria-label="Files queued for upload">
              {pendingFiles.map((pf) => (
                <li
                  key={pf.id}
                  className="rounded-lg bg-amber-900/20 border border-amber-900/30 px-3 py-2"
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {pf.status === "error" && (
                        <span className="text-[10px] text-red-400" title={pf.errorMsg}>
                          ✗ Failed
                        </span>
                      )}
                  ...
                  {pf.status === "uploading" && (
                    <div
                      className="mt-1.5 h-1 rounded-full bg-amber-900/40 overflow-hidden"
                      role="progressbar"
          ...
          {pendingFiles.some((f) => f.status === "pending") && (
            <button type="button" onClick={uploadPending} className="btn-primary text-sm w-full">
