...
const STEPS = [
  {
    ...
    desc: "Describe the work, set an XLM budget, and post. Budget is locked in Soroban escrow instantly.",
  },
  {
    icon: "🙋",
    title: "Freelancers apply",
    desc: "Skilled contributors submit proposals and bid amounts. Client reviews and picks the best fit.",
  },
  {
    icon: "🔒",
    title: "Work begins in escrow",
    desc: "Funds sit safely in a smart contract. Freelancer starts work knowing payment is guaranteed.",
  },
  {
    icon: "✅",
    title: "Client approves & pays",
    desc: "Once work is approved, one click releases the escrowed XLM directly to the freelancer.",
  },
];
...
function StatCard({
  ...
  stat: { value: number; suffix: string; label: string; duration: number; prefix: string };
  index: number;
}) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            {publicKey ? (
              <>
                ...
                <Link href="/post-job" className="btn-secondary text-base px-8 py-3.5">
                  Post a Job
                </Link>
              </>
            ) : (
        ...
        <div className="mb-20">
          <div className="text-center mb-12">
            ...
            <p className="text-amber-800 max-w-xl mx-auto font-body">
              Four steps from job post to payment. No banks. No waiting. No trust required.
            </p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {STEPS.map((step, i) => (
              <div
                key={step.title}
                className="card group hover:border-market-500/25 transition-all"
              >
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-2xl">{step.icon}</span>
                  <span className="text-xs font-mono text-market-500/60 font-medium">
                    Step {i + 1}
                  </span>
                </div>
                <h3 className="font-display font-semibold text-amber-200 mb-2 text-base">
                  {step.title}
                </h3>
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-amber-800 text-sm font-body">
                Real work, real payments — settled on Stellar.
              </p>
            </div>
            <Link
          ...
          {completedJobs.length === 0 ? (
            <div className="card text-center py-12">
              ...
              <p className="text-amber-700 font-body text-sm">
                No completed jobs yet — be the first to finish one.
              </p>
            </div>
          ) : (
        ...
        <div className="card mb-20 bg-gradient-to-br from-ink-800 to-ink-900 border-market-500/18">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-amber-700 leading-relaxed mb-6 font-body">
              Traditional freelance platforms take 10–20% of every payment and hold funds for weeks.
              Stellar transactions settle in seconds for a fraction of a cent — and Soroban smart
              contracts enforce payment release automatically, so neither party needs to trust the
              other.
            </p>
            <div className="grid sm:grid-cols-3 gap-4 text-sm">
              {[
                ...
                <div
                  key={f.label}
                  className="bg-ink-900/60 rounded-xl p-4 border border-market-500/10"
                >
        ...
        <div className="text-center pb-12 border-t border-market-500/8 pt-8">
          <p className="text-amber-900 text-sm font-body">
            ...
            <a
              href="https://github.com/your-org/stellar-marketpay"
              target="_blank"
