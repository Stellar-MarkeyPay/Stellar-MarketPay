    ...
    ({ mockJobs, mockJob, publicKey }) => {
      ...
      XMLHttpRequest.prototype.send = function (body) {
        ...
        if (method === "GET" && url.includes("/api/")) {
          let data: any = null;
          if (url.includes("/api/jobs/recommended"))
            data = []; // No recommendations for core tests
          ...
          else if (url.includes("/client-reputation"))
            data = {
              score: 4.5,
            ...
            data = { transaction: "challenge-xdr", success: true };
          else data = [];
          ...
          setTimeout(() => {
            ...
            Object.defineProperty(xhr, "responseText", {
              value: JSON.stringify({ success: true, data }),
              configurable: true,
            });
        ...
        if (method === "POST" && url.includes("/api/auth")) {
          ...
          setTimeout(() => {
            ...
            Object.defineProperty(xhr, "status", { value: 200, configurable: true });
            Object.defineProperty(xhr, "responseText", {
              value: JSON.stringify({ success: true, token: "jwt-token" }),
              configurable: true,
            });
            xhr.dispatchEvent(new Event("readystatechange"));
            xhr.dispatchEvent(new Event("load"));
