...
export default function StateMessage({
  ...
  const bgClass =
    type === "error" ? "bg-red-500/10 border-red-500/20" : "bg-amber-500/10 border-amber-500/20";
  ...
  return (
    <div className={clsx("card text-center py-16 border", borderClass, bgClass)}>
      {icon && <div className="mb-4 flex justify-center animate-pulse">{icon}</div>}
      ...
      {ctaLabel && onCta && (
        <button
          className={clsx(
            "btn-primary text-sm",
            type === "error" ? "bg-red-500/30 hover:bg-red-500/50" : ""
          )}
          onClick={onCta}
        >
          {ctaLabel}
