...
class ChaosInjector {
  ...
  off(eventType: any, handler: any) {
    this.eventListeners = this.eventListeners.filter(
      (listener: any) => !(listener.type === eventType && listener.handler === handler)
    );
  }

  emit(eventType: any, data: any) {
    this.eventListeners
      .filter((listener: any) => listener.type === eventType)
      .forEach((listener: any) => listener.handler(data));
  }

  createLatencyInjectedQuery(originalQuery: any, latencyMs: any) {
  ...
  createTimeoutInjectedQuery(originalQuery: any, timeoutMs: any) {
    return async (...args: any[]) => {
      return Promise.race([
        originalQuery(...args),
  ...
  createConnectionLossQuery(originalQuery: any, failureRate = 0.5) {
    return async (...args: any[]) => {
  ...
  createErrorInjectedQuery(originalQuery: any, errorRate = 0.1) {
    return async (...args: any[]) => {
  ...
  async runChaosScenario(scenarioName: any, testFn: any, iterations = 1) {
    const results = [];
    let successCount = 0;
    let errorCount = 0;
    const latencies = [];
