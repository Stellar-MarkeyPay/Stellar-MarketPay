...
const realtimeClients: any = new Set();
const scopeSessionClients: any = new Map();

function broadcastRealtime(event: any, payload: any) {
...
async function upsertScopeSession(sessionId: any, patch: any) {
...
async function loadScopeSession(sessionId: any) {
...
async function cleanupExpiredScopeSessions() {
  try {
    const result = await rawQuery<ScopeSessionTable>(
...
app.get("/metrics", async (req: any, res: any, next: any) => {
  try {
...
function sendJson(ws: any, event: any, payload: any) {
  if (ws.readyState === WS_OPEN) {
    ws.send(JSON.stringify({ event, payload }));
  }
}

function getScopeSessionSet(sessionId: any) {
...
wsServer.on("connection", async (ws: any, request: any) => {
  const url = new URL(request.url, `http://${request.headers.host}`);
...
async function startNotificationProcessor() {
  ...
  const sendEmailFn = async ({ to, subject, text, html }: any) => {
    if (!smtpTransport || !to) return;
    await smtpTransport.sendMail({
      from: process.env.SMTP_FROM || process.env.SMTP_USER,
      to,
