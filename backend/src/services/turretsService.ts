 ...
 */
async function submitViaTurret(transactionXDR: any, options = {}) {
  if (!transactionXDR) {
 ...
 */
async function estimateTurretFee(transactionXDR: any) {
  if (!transactionXDR) {
    const e = new Error("Transaction XDR is required");
    e.status = 400;
    throw e;
 ...
 */
function shouldUseTurret(options: any = {}) {
