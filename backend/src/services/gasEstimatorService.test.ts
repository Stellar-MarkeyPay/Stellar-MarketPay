...
function makeFeeStats(overrides = {}) {
  return {
    fee_charged: {
      ...
      p50: "400",
      p95: "1000",
      ...(overrides as any).fee_charged,
    },
    soroban_inclusion_fee_charged: {
      p10: "200",
      p50: "600",
      p90: "1500",
      ...(overrides as any).soroban_inclusion_fee_charged,
    },
    ...overrides,
  };
}
...
describe("computeTiers", () => {
  ...
  it("each tier exposes feeXlm as a string", () => {
    ...
    for (const tier of Object.values(tiers)) {
      expect(typeof (tier as any).feeXlm).toBe("string");
      expect((tier as any).feeXlm.length).toBeGreaterThan(0);
    }
  });
...
describe("getGasEstimate", () => {
  ...
  it("stores result in cache after fetching", async () => {
    ...
    expect(cacheService.set.mock.calls.some((c: any) => c[0] === "gas:estimate:v1")).toBe(true);
  });
