 ...
 */
async function predictJobCompletion(job: any, freelancerAddress = null) {
  let completedJobs = 0;
  let rating = 4.0;
  ...
  if (freelancerAddress) {
    ...
    for (const h of historyRows) {
      if (!h.deadline) {
        ...
        const completedAt = new Date(h.updated_at as any);
