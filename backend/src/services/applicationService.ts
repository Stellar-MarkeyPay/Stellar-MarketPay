...
async function submitApplication({
  ...
  if (job.visibility === "invite_only") {
    const { rows: inviteRows } = await rawQuery<JobInvitationTable>(
...
async function closeBiddingForJob(jobId: string, clientAddress: string) {
...
function hashBidReveal(bidAmount: string, nonce: string) {
  return crypto
    .createHash("sha256")
...
async function revealApplicationBid(
  ...
  const { rows } = await rawQuery<ApplicationTable>("SELECT * FROM applications WHERE id = $1", [
    applicationId,
  ]);
  if (!rows.length) {
  ...
  const { rows: updatedRows } = await rawQuery<ApplicationTable>(
    `UPDATE applications
 ...
 */
async function getApplicationsForJob(jobId: string, filters: any = {}) {
  const { rows } = await rawQuery<ApplicationTable>(
    `SELECT a.*,
  ...
  return applications.filter((application: any) => application.freelancerTier === filters.tier);
}
 ...
 */
async function getApplicationsForFreelancer(freelancerAddress: string) {
  validatePublicKey(freelancerAddress);
  const { rows } = await rawQuery<ApplicationTable>(
    `SELECT a.*,
            COALESCE(p.completed_jobs, 0) AS completed_jobs,
 ...
 */
async function acceptApplication(applicationId: string, clientAddress: string) {
  validatePublicKey(clientAddress);
 ...
 */
async function withdrawApplication(applicationId: string, freelancerAddress: string) {
  validatePublicKey(freelancerAddress);
  ...
  const { rows: updated } = await rawQuery<ApplicationTable>(
