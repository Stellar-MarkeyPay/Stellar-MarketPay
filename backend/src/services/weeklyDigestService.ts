 ...
 * @param {number} maxLen
 */
function excerpt(text: string, maxLen: number = 120) {
  if (!text) return "";
 ...
 */
function formatBudget(budget: string | number, currency: string = "XLM") {
  const num = parseFloat(String(budget));
 ...
 */
function buildJobCard(job: any, baseUrl: string) {
 ...
 */
function generateDigestEmail(jobs: any[], unsubToken: string, baseUrl: string, apiBaseUrl: string) {
  ...
  // ── HTML ─────────────────────────────────────────────────────────────────
  const jobCards = jobs.map((j: any) => buildJobCard(j, baseUrl)).join("");
 ...
 */
async function sendWeeklyDigest(sendEmailFn: any) {
  ...
  for (const freelancer of freelancers) {
    ...
    try {
      ...
      const recentMatches = allMatches
        .filter((j: any) => new Date(j.created_at) >= oneWeekAgo)
        .slice(0, 5);
      ...
      const { subject, text, html } = generateDigestEmail(
        recentMatches,
        digest_unsubscribe_token as unknown as string,
        baseUrl,
        apiBaseUrl
      );
