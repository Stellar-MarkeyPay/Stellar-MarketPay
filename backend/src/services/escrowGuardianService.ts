...
async function setGuardian(jobId: any, guardianAddress: any, highValueThreshold: any) {
  try {
    const result = await rawQuery<EscrowTable>(
      `UPDATE escrows
...
async function approveRelease(jobId: any, guardianAddress: any) {
  try {
    const escrow = await rawQuery<EscrowTable>(`SELECT * FROM escrows WHERE job_id = $1`, [jobId]);
...
async function canReleaseEscrow(jobId: any, releaserAddress: any) {
  try {
    const escrow = await rawQuery<EscrowTable>(
