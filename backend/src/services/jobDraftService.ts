...
async function saveDraft(clientAddress: any, draftData: any) {
  const {
    id,
    title,
    description,
  ...
  const result = await rawQuery<any>(query, values);
  return result.rows[0];
}

async function getDrafts(clientAddress: any, limit = 5) {
  const query = `
    ...
    LIMIT $2
  `;
  const result = await rawQuery<any>(query, [clientAddress, limit]);
  return result.rows;
}

async function getDraft(draftId: any, clientAddress: any) {
  const query = `
    SELECT * FROM job_drafts
    WHERE id = $1 AND client_address = $2
  `;
  const result = await rawQuery<any>(query, [draftId, clientAddress]);
  return result.rows[0];
}

async function deleteDraft(draftId: any, clientAddress: any) {
  const query = `
    DELETE FROM job_drafts
    WHERE id = $1 AND client_address = $2
  `;
  await rawQuery<any>(query, [draftId, clientAddress]);
}
