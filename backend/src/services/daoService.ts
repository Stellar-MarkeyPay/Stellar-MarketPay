...
async function listProposals({ status }: any = {}) {
  const conditions = [];
  const params = [];
  if (status) {
    params.push(status);
...
async function getProposal(id: any) {
  const { rows } = await rawQuery<any>(
    `SELECT p.*,
       COALESCE(SUM(v.weight) FILTER (WHERE v.support = true), 0) AS votes_for,
       COALESCE(SUM(v.weight) FILTER (WHERE v.support = false), 0) AS votes_against,
...
async function castVote({ proposalId, voter, support, weight, txHash }: any) {
...
async function upsertArbitrator({ publicKey, displayName, bio }: any) {
...
async function voteForArbitrator({ voter, arbitratorKey, weight }: any) {
  validatePublicKey(voter);
