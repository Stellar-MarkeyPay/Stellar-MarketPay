 ...
 */
async function inviteFreelancerToJob({ jobId, clientAddress, freelancerAddress }: any) {
 ...
 */
async function getInvitationsForFreelancer(freelancerAddress: any) {
 ...
 * @returns {Promise<Object>}
 */
async function declineInvitation(invitationId: any, freelancerAddress: any) {
  const { rows } = await rawQuery<JobInvitationTable>(
  ...
  const { rows: updated } = await rawQuery<JobInvitationTable>(
