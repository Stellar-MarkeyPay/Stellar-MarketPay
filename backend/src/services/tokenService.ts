 ...
 */
async function getTokenMetadata(contractId: any) {
  if (!contractId) {
 ...
 */
async function getTokenBalance(publicKey: any, contractId: any) {
  ...
  try {
    ...
    const tokenBalance = account.balances.find(
      (balance: any) =>
        balance.asset_code && balance.asset_issuer && balance.asset_issuer === contractId
    );
 ...
 */
async function validateTokenContract(contractId: any) {
  if (!contractId) {
    const e = new Error("Contract ID is required");
    e.status = 400;
    throw e;
 ...
 */
async function searchTokens(query: any) {
