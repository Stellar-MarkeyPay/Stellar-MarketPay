...
function validatePublicKey(key: any) {
...
async function logTimeEntry({
  ...
  const { rows } = await rawQuery<TimeEntryTable>(
 ...
 */
async function getTimeEntriesForJob(jobId: any) {
  if (!jobId) {
    const e = new Error("jobId is required");
    e.status = 400;
    throw e;
 ...
 */
async function generateInvoice({ jobId, freelancerAddress, hourlyRateXlm, entryIds }: any) {
  validatePublicKey(freelancerAddress);
  ...
  const totalMinutes = entries.reduce((sum: any, e: any) => sum + e.duration_minutes, 0);
  const totalHours = totalMinutes / 60;
  const totalAmountXlm = (totalHours * rate).toFixed(7);
  const includedIds = entries.map((e: any) => e.id);

  const { rows: invoiceRows } = await rawQuery<TimeInvoiceTable>(
 ...
 */
async function getInvoicesForJob(jobId: any) {
  if (!jobId) {
    const e = new Error("jobId is required");
    e.status = 400;
    throw e;
  ...
  const { rows } = await rawQuery<TimeInvoiceTable>(
 ...
 */
async function reviewInvoice({ invoiceId, clientAddress, decision, contractTxHash }: any) {
  ...
  const { rows: invRows } = await rawQuery<TimeInvoiceTable>(
  ...
  const { rows: updated } = await rawQuery<TimeInvoiceTable>(
