...
class IndexerService {
  ...
  async saveCheckpoint({ ledger, txTimestamp, synced = true }: any) {
    await rawQuery<any>(
  ...
  async processEvent(event: any) {
    ...
    const eventType = (typeMap as any)[eventTypeRaw];
    if (!eventType) return;
    ...
    if (this.cdnInvalidation) {
      ...
      this.cdnInvalidation
        .handleContractEvent(eventType, jobId, { receivedAt })
        .catch((error: any) => {
  ...
  async getEventsForJob(jobId: string) {
    const { rows } = await rawQuery<any>(
