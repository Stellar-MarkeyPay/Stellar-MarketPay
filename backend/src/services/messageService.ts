 ...
 */
async function verifyJobParticipant(jobId: any, userAddress: any) {
 ...
 */
async function createMessage({ jobId, senderAddress, content, contractTxHash }: any) {
 ...
 */
async function getMessagesByJob(jobId: any, userAddress: any) {
  ...
  const { rows } = await rawQuery<MessageTable>(
 ...
 */
async function markMessagesAsRead(jobId: any, userAddress: any) {
  await rawQuery<MessageTable>(
    `UPDATE messages
     SET read = TRUE
     WHERE job_id = $1
 ...
 */
async function getUnreadCount(userAddress: any) {
  const { rows } = await rawQuery<MessageTable>(
 ...
 */
async function attachTxHash(messageId: any, txHash: any) {
  const { rows } = await rawQuery<MessageTable>(
    `UPDATE messages
     SET tx_hash = $1
     WHERE id = $2
