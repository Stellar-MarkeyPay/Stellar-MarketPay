...
function generateSecret(adminId: any) {
...
async function ensureAdminProfile(adminId: any) {
  await rawQuery<any>(
...
async function getDecryptedSecret(adminId: any) {
...
async function enable2FA(adminId: any, plainSecret: any, backupCodes: any) {
...
async function verify2FA(adminId: any, token: any) {
  ...
  if (verified) {
    await rawQuery<any>("UPDATE admin_profiles SET totp_attempts = 0 WHERE id = $1", [adminId]);
    return { success: true };
  }
  ...
  await rawQuery<any>("UPDATE admin_profiles SET totp_attempts = $1 WHERE id = $2", [
    newAttempts,
    adminId,
  ]);
...
async function verifyBackupCode(adminId: any, code: any) {
  const { rows } = await rawQuery<any>("SELECT backup_codes FROM admin_profiles WHERE id = $1", [
    adminId,
  ]);
  const admin = rows[0];
  ...
  await rawQuery<any>("UPDATE admin_profiles SET backup_codes = $1 WHERE id = $2", [
...
async function disable2FA(adminId: any) {
  await rawQuery<any>(
    `UPDATE admin_profiles
...
async function get2FAStatus(adminId: any) {
