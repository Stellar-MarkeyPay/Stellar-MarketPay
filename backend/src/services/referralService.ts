 ...
 */
async function registerReferral(referrerAddress: string, refereeAddress: string) {
  validatePublicKey(referrerAddress);
  validatePublicKey(refereeAddress);

  if (referrerAddress === refereeAddress) {
    const e = new Error("Referrer and referee cannot be the same address") as CustomError;
    e.status = 400;
    throw e;
  }
  ...
  for (let i = 0; i < MAX_DEPTH + 1; i++) {
    ...
    if (cursor === refereeAddress) {
      const e = new Error("Registering this referral would create a cycle") as CustomError;
      e.status = 400;
      throw e;
    }
  }
  ...
  const { rows: parentRows } = await rawQuery<any>(
  ...
  try {
    ...
    if ((err as CustomError).code === "23503") return null; // FK violation = no profile yet
    throw err;
  } finally {
    client.release();
  }
 ...
 */
async function getReferrerForReferee(refereeAddress: string) {
  const { rows } = await rawQuery<any>(
    `SELECT parent_address FROM referral_tree WHERE child_address = $1 LIMIT 1`,
    [refereeAddress]
  );
 ...
 */
async function processMultiLevelPayout(
  jobId: string,
  ...
  const { rows: prevJobs } = await rawQuery<EscrowTable>(
    `SELECT COUNT(*) AS cnt
     FROM escrows e
 ...
 */
async function processPlatformFeePayout(
  jobId: string,
 ...
 */
async function getReferralStats(publicKey: string) {
  validatePublicKey(publicKey);

  // Summary counts (legacy referrals table)
  const { rows: summary } = await rawQuery<ReferralTable>(
    `SELECT
  ...
  const { rows: treeEarnings } = await rawQuery<any>(
  ...
  const { rows: feeEarnings } = await rawQuery<any>(
  ...
  const { rows: referees } = await rawQuery<ReferralTable>(
    `SELECT
       r.id,
       r.referee_address,
       r.status,
  ...
  const { rows: payouts } = await rawQuery<ReferralPayoutTable>(
    `SELECT
       rp.id,
       rp.referee_address,
       rp.job_id,
 ...
 */
async function getReferralTree(publicKey: string) {
  validatePublicKey(publicKey);
  ...
  const { rows: rootProfile } = await rawQuery<ProfileTable>(
    "SELECT display_name FROM profiles WHERE public_key = $1",
    [publicKey]
  );
  if (rootProfile.length) (root as any).displayName = rootProfile[0].display_name;
