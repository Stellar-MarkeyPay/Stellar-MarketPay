 ...
 */
function calculatePremium(fileSize: any, fileValue: any) {
 ...
 */
async function createInsuredFile(
  cid: string,
  ownerAddress: string,
  fileSize: number,
  fileValue: number,
  ...
  const result = await rawQuery<any>(query, params);
 ...
 */
async function checkIPFSAvailability(cid: string) {
  try {
 ...
 * @returns {Promise<boolean>} True if file is available
 */
async function checkArweaveAvailability(txId: string) {
  try {
 ...
 */
async function updateAvailabilityMetrics(fileId: string, isAvailable: boolean) {
  const query = `
  ...
  const result = await rawQuery<any>(query, [isAvailable ? 1 : 0, fileId]);
 ...
 */
async function performAvailabilityCheck(fileId: string) {
  try {
    // Get file details
    const fileQuery = "SELECT * FROM insured_files WHERE id = $1";
    const fileResult = await rawQuery<any>(fileQuery, [fileId]);
 ...
 */
async function evaluateInsuranceClaim(fileId: string) {
  try {
    const fileQuery = "SELECT * FROM insured_files WHERE id = $1";
    const fileResult = await rawQuery<any>(fileQuery, [fileId]);
    ...
    const existingClaim = await rawQuery<any>(existingClaimQuery, [fileId]);
    ...
    const claimResult = await rawQuery<any>(claimQuery, [
      fileId,
      file.owner_address,
      claimAmount,
      "pending",
 ...
 */
async function submitOracleProof(claimId: string, oracleProof: string, oracleAddress: string) {
  try {
    const query = `
    ...
    const result = await rawQuery<any>(query, [oracleProof, oracleAddress, claimId]);
 ...
 */
async function approveInsuranceClaim(claimId: string, txHash: string) {
  try {
    const query = `
      UPDATE insurance_claims
      SET
    ...
    const result = await rawQuery<any>(query, [txHash, claimId]);
 ...
 */
async function rejectInsuranceClaim(claimId: string, reason: string) {
  try {
    const query = `
      UPDATE insurance_claims
      SET
    ...
    const result = await rawQuery<any>(query, [reason, claimId]);
 ...
 */
async function getUserInsuredFiles(ownerAddress: string) {
  const query = `
    SELECT
  ...
  const result = await rawQuery<any>(query, [ownerAddress]);
  return result.rows;
}
...
async function getInsuranceClaim(claimId: string) {
  ...
  const result = await rawQuery<any>(query, [claimId]);
...
async function getInsuranceStats() {
  ...
  const result = await rawQuery<any>(statsQuery);
