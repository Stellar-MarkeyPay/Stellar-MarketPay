...
async function getPriceAlertPreference(freelancerAddress: any) {
  validatePublicKey(freelancerAddress);
  const { rows } = await rawQuery<any>(
...
function throwBadRequest(message: any) {
...
class PriceAlertService {
  ...
  async runOnce() {
    ...
    const { rows } = await rawQuery<any>("SELECT * FROM price_alert_preferences");
  ...
  async handleTrigger(pref: any, kind: any, currentPriceUsd: any, threshold: any) {
