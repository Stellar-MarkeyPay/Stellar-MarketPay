...
class CdnPurgeError extends Error {
  [key: string]: any;
  constructor(message: any, attempts: any) {
    super(message);
...
class CdnService {
  ...
  constructor({
    ...
    this.circuits = new Map(providers.map((p: any) => [p.name, { failures: 0, openUntil: 0 }]));
  ...
  _recordSuccess(name: any) {
    const circuit = this.circuits.get(name);
    circuit.failures = 0;
    circuit.openUntil = 0;
  }

  async _withTimeout(promise: any, ms: any, providerName: any) {
    let timer;
