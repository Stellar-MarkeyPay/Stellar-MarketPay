...
function normalizePositiveNumber(value: any, message: string) {
...
function pruneEntries(entries: any[], now: number, windowMs: number) {
...
function createAlert({
  ...
  const primaryRule = rules.sort((a: any, b: any) => b.riskScore - a.riskScore)[0];
  const now = Date.now();
...
function shouldSuppressAlert(alert: any) {
...
async function analyzeBidEvent(input: any = {}) {
  const {
    jobId,
    applicationId,
    freelancerAddress,
  ...
  if (jobStats.count >= RULES.minJobBidsForAmountStats && (jobStats as any).stdDev > 0) {
    const zScore = Math.abs((amount - (jobStats as any).mean) / (jobStats as any).stdDev);
      ...
      rules.push(
        createRule(
          ...
          Math.min(95, 50 + zScore * 10),
          {
            ...
            meanBid: Number((jobStats as any).mean.toFixed(7)),
            stdDevBid: Number((jobStats as any).stdDev.toFixed(7)),
...
function getJobFraudStats(jobId: string) {
  if (!jobId) {
    const error = new Error("jobId is required");
    error.status = 400;
    throw error;
  ...
  return {
    ...
    mean: (jobStats as any).mean == null ? null : Number((jobStats as any).mean.toFixed(7)),
    min: jobStats.min == null ? null : formatAmount(jobStats.min),
    max: jobStats.max == null ? null : formatAmount(jobStats.max),
    stdDev: (jobStats as any).stdDev == null ? null : Number((jobStats as any).stdDev.toFixed(7)),
    rules: RULES,
    flaggedFreelancers,
  };
}
