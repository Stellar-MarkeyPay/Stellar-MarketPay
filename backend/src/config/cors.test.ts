...
function checkOrigin(options: any, origin: any) {
  return new Promise((resolve) => {
    options.origin(origin, (error: any, allowed: any) => {
...
describe("CORS configuration", () => {
  it("denies cross-origin requests in production when ALLOWED_ORIGINS is unset", async () => {
    ...
    expect((result as any).allowed).toBeUndefined();
    expect((result as any).error).toEqual(new Error("CORS blocked"));
  ...
  it("only allows configured origins in production", async () => {
    ...
    expect((rejected as any).allowed).toBeUndefined();
    expect((rejected as any).error).toEqual(new Error("CORS blocked"));
  });
