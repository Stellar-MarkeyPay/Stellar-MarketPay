...
router.get("/info", (req: any, res: any) => {
  res.json({
    ...
    data: {
      ...
      levelBps: LEVEL_BPS,
      levels: LEVEL_BPS.map((bps: any, i: any) => ({
        level: i + 1,
        bps,
      ...
      description: `Earn up to ${LEVEL_BPS.reduce((a: any, b: any) => a + b, 0) / 100}% in multi-level referral bonuses`,
 ...
 */
router.get("/:publicKey", verifyJWT, generalRateLimiter, async (req: any, res: any, next: any) => {
  try {
 ...
 *         $ref: '#/components/responses/TooManyRequests'
 */
router.post("/register", generalRateLimiter, async (req: any, res: any, next: any) => {
  try {
