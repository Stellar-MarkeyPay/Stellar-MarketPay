...
router.get("/proposals", daoRateLimiter, async (req: any, res: any, next: any) => {
  try {
...
router.get("/proposals/:id", daoRateLimiter, async (req: any, res: any, next: any) => {
  try {
...
router.post("/proposals", verifyJWT, daoRateLimiter, async (req: any, res: any, next: any) => {
  try {
...
router.get("/treasury", daoRateLimiter, async (req: any, res: any, next: any) => {
  try {
...
router.get("/arbitrators", daoRateLimiter, async (req: any, res: any, next: any) => {
  try {
...
router.get("/arbitrators/:publicKey", daoRateLimiter, async (req: any, res: any, next: any) => {
  try {
    const arbitrators = await daoService.listArbitrators();
    const found = arbitrators.find((a: any) => a.publicKey === req.params.publicKey);
    if (!found) {
...
router.post("/arbitrators", verifyJWT, daoRateLimiter, async (req: any, res: any, next: any) => {
  try {
