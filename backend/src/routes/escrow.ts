 ...
 *               $ref: '#/components/schemas/Error'
 */
router.post("/:jobId/release", async (req: any, res: any, next: any) => {
  try {
 ...
 *               $ref: '#/components/schemas/Error'
 */
router.post("/:jobId/refund", async (req: any, res: any, next: any) => {
  try {
    const { jobId } = req.params;
    const { clientAddress, contractTxHash } = req.body;
    const job = await getJob(jobId);
 ...
 *               $ref: '#/components/schemas/Error'
 */
router.post("/:jobId/timeout-refund", async (req: any, res: any, next: any) => {
  try {
    const { jobId } = req.params;
    const { clientAddress, contractTxHash } = req.body;
    const job = await getJob(jobId);
 ...
 *         $ref: '#/components/responses/TooManyRequests'
 */
router.get("/:jobId", escrowActionRateLimiter, async (req: any, res: any, next: any) => {
  try {
    const { rows } = await rawQuery<EscrowTable>("SELECT * FROM escrows WHERE job_id = $1", [
      req.params.jobId,
    ]);
