...
router.get("/:skill", verifyJWT, async (req: any, res: any, next: any) => {
  try {
    ...
    const safeQuestions = bank.questions.map(({ id, question, options }: any) => ({
      id,
      question,
      options,
    }));
...
router.post("/:skill/submit", verifyJWT, async (req: any, res: any, next: any) => {
  try {
    const skill = req.params.skill.toLowerCase();
    const bank = questions[skill];
    if (!bank) return res.status(404).json({ error: "Unknown skill" });
    ...
    const { rows: prev } = await rawQuery<any>(
    ...
    if (passed) {
      ...
      // Store certificate
      const { rows: certRows } = await rawQuery<any>(
...
router.get("/results/:publicKey", async (req: any, res: any, next: any) => {
  try {
    const { rows } = await rawQuery<any>(
