...
router.post("/job/:jobId", verifyJWT, generalRateLimiter, async (req: any, res: any, next: any) => {
  try {
...
router.get("/job/:jobId", verifyJWT, generalRateLimiter, async (req: any, res: any, next: any) => {
  try {
    const { jobId } = req.params;
